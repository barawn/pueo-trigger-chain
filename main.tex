...
to coherently sum up to 8 antennas in multiple (48 implemented) beamformed
...
split between horizontal and vertical polarizations. An additional 2 SURFs handle a
...
Corporation specifically for PUEO, focusing on the $300-1200~\mathrm{MHz}$ frequency
range. The antennas are dual polarization, with the horizontal and vertical
polarizations both independently recorded and triggered by two separate data
...
The trigger was split into multiple levels, with individual beam triggers merged into
...
block are based on the same 31-tap finite impulse response halfband filter.
...
This is due to a combination of the off-axis gain profile of the antennas
...
to decimate by a factor of 2 and operate at $1500\,\text{MSa/s}$ to 
reduce power. However, before the AGC-BR block, the signal is restored
