...
gate array (FPGA) termed the PL, and a quad-core ARM processing system termed the PS. 
...
\begin{center}
\includegraphics[width=0.5\textwidth]{plots/1000022656.jpg}
\end{center}
...
processing blocks to maximize the the instrument's detection capability,
as shown in Fig.~\ref{triggerfilterchain}. The filter chain consists of (in order)
a downsampling block, a matched filter, a CW rejection block, an
...
filters. Due to power management needs, builds containing 0, 1, or 2 biquads are
selectable during normal operation. Both the upsampling and downsampling
blocks are based on the same 31-tap finite impulse response halfband filter.

\begin{figure}
\begin{center}
\includegraphics[width=0.9\linewidth]{plots/signal_flow.drawio.pdf}    
\end{center}
...
Fig.~\ref{fig:halfbandStructure}. The notation $x[i]=x z^{i}$ has been chosen 
to represent calculations in supersample rate; note that system clocks 
generate a delay of $z^{-M}$, where $M=8$.
...
$H_p\left(z\right)^{-1}=1-2P\cos\theta z^{-1}+P^{2}z^{-2}$.
...
implementation for PUEO uses $M=4$, however the filter itself is
parameterized for any $M$ and was  originally tested with $M=8$ in a
...
This overall method is similar to that described by Bui et al.
...
produced using a custom calibration instrument built around a Xilinx ZCU216 RFSoC evaluation board (ZCU216). 
...
beams of both of the SURFs under test. 

\begin{figure}
    \begin{center}
    \includegraphics[width=0.9\textwidth]{plots/efficiency_errFit.png}        
    \end{center}
    \caption{\label{fig:efficiency_beamformer} PUEO trigger efficiency, calculated from data taken in the launch facility
