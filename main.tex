...
added using ternary adder structures. The unused addend in the
...
summed signal now ranges over $\pm124$ for the 8-antenna beams
and $\pm93$ for the 6-antenna beams.
...
\cite{bui2014additional} except for signed cases, but the 
partial-product terms chosen to combine were optimized to ensure
that no more than 4 inputs were used at any stage of the ternary 
adder requiring the propagated carry, ensuring that the partial
product calculations could be fully internal to the adder LUTs.
While other optimized squarers for FPGAs have been presented 
\cite{bottcher2022resource}, these were focused on building 
efficient larger-width squarers or for direct ASIC implementation,
and the detailed size of small squarers was not clear. The aggressive
optimization here is most appropriate because of the large number of
squares ($384~\mathrm{total}$) required - the optimized square reduced 
the slice count of the beamforming trigger by $56\%$ alone.
...
these are implemented as direct LUTs. The squarer fits into 2 UltraScale
slices, a total of 4 LUTs plus the adder structure.}
\end{figure}
...
which physically corresponds to a $30^\circ$ section of the
